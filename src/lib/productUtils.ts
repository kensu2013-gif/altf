...
// Helper: Find matching product in inventory
export const findMatchingProduct = (item: LineItem, inventoryList: Product[]) => {
    const normalize = (val: string | number) => String(val).replace(/\s+/g, '').toUpperCase();

    // Priority 1: ID Construction Match (Exact & Robust)
    const candidateId = generateSku(item);
    let product = inventoryList.find(p => p.id === candidateId || normalize(p.id) === normalize(candidateId));

    // Priority 2: Robust Property Match
    if (!product) {
        product = inventoryList.find(p =>
            normalize(p.name) === normalize(item.name) &&
            normalize(p.thickness) === normalize(item.thickness) &&
            normalize(p.size) === normalize(item.size) &&
            normalize(p.material) === normalize(item.material)
        );
    }

    // Priority 3: Existing Product ID (if plausible) logic
    if (!product && item.productId) {
        product = inventoryList.find(p => p.id === item.productId || normalize(p.id) === normalize(item.productId!));
    }

    return product || null;
};

export function getSharedMaterialColor(m: string): string {
    if (!m) return "";
