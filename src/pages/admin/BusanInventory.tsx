...
const LEAD_TIME = 5;           // 리드타임 (대경 → 부산, 영업일 기준)
const Z_VALUE = 1.645;         // 목표 서비스율 95% (데이터 충분하면 2.33 = 99%로 상향)

export type DkProcurementFilterType = 'ALL' | 'ORDER_NEEDED' | 'RECOMMENDED' | 'DOUBLE_STOCKOUT' | 'SURGING_DEMAND' | 'SURGING_MONITORING' | 'SURGING_RECOMMENDED' | 'SIHWA_UNMET' | 'EXCESS' | 'STABLE' | 'SURGE_ITEM' | 'DROP_ITEM';
...
export default function BusanInventory() {
    ...
    const { inventory: rawInventory, isLoading: invLoading, refresh: refreshInventory } = useInventory();
    const inventory = useMemo(() => {
        return rawInventory.map(item => ({
            ...item,
            shQty: item.locationStock?.['부산'] || 0
        }));
    }, [rawInventory]);
    ...
    const isKyunggiCompany = useCallback((
        ...
        const addrSource = u?.address || orderOrQuote?.customerAddress || orderOrQuote?.buyerInfo?.address || orderOrQuote?.customerInfo?.address || '';
        const regInfo = getCustomerRegion(addrSource);
        return regInfo.region === 'SIHWA';
    }, [userMap]);
    ...
    // 부산 재고관리 일괄/선택 발주 수량 확인/수정 팝업 모달 상태
    ...
    const sihwaOrders = useMemo(() => {
        return activeOrders.filter(order => {
            ...
            return normalizedCustomer.includes('재고') ||
                ...
                normalizedCustomer.includes('부산') ||
        ...
        [...historyData.inventoryHistory].forEach(snap => {
            ...
            (snap.diff || []).forEach(d => {
                ...
                const isTargetLocation = locStr.includes(targetRegion) || locStock[targetRegion] !== undefined;
                                const isTargetMaker = product.maker === '태일' || product.maker1 === '태일' || product.maker === '대경' || product.maker1 === '대경';
    ...
    }, [historyData.inventoryHistory, inventoryMap, targetRegion, searchTerm, sihwaFilterItem, sihwaFilterMaterial, sihwaFilterSize, sihwaFilterThickness]);
    ...
    const daekyungAnalysis = useMemo(() => {
        ...
        return computeDaekyungStockAnalysis(targetProducts, historyData.daekyungHistory);
    }, [inventory, historyData.daekyungHistory]);

    const daekyungBaseStockAverages = daekyungAnalysis.items;
    const daekyungCoverage: DaekyungCoverageStats = daekyungAnalysis.coverage;

    const daekyungDropItems = useMemo(() => {
        return daekyungBaseStockAverages
            .filter(r => r.anomalyType === 'DROP')
            .sort((a, b) => a.changePct1m - b.changePct1m)
            .slice(0, 5);
    }, [daekyungBaseStockAverages]);

    const daekyungSurgeItems = useMemo(() => {
        return daekyungBaseStockAverages
            .filter(r => r.anomalyType === 'SURGE')
            .sort((a, b) => b.changePct1m - a.changePct1m)
            .slice(0, 5);
    }, [daekyungBaseStockAverages]);
    ...
    const baseAnalyzedInventory = useMemo(() => {
        ...
        inventory.forEach((item: Product) => {
            ...
            if (item.locationStock) {
                if (item.locationStock['부산'] !== undefined) shQty += Number(item.locationStock['부산']);
                ...
                if ((item.location || '').includes('부산') || (item.location || '').includes('서울')) {
        ...
        const processedList = Object.values(comparisonMap).map(row => {
            ...
            // 부산의 실제 평균 출고량 추정 (최근 60일 부산 출고량 기준)
            ...
            // 2. 최대 발주 상한선 캡 (부산에서 한 번에 나가는 물량은 보통 200~300개를 넘지 않으므로 최대 500개 캡, 단, 월판매량이 아주 큰 경우는 2개월치 허용)
            ...
            // 3.5. 대경 직발주 비중이 압도적으로 높고 부산 출고가 없는 경우, 기초 재고 10개 강제 보류 해제
            ...
            // 부산에서 보통 한 번에 200~300개씩 입고하므로, 단순히 safeStock의 1.5배로 잡으면 대다수가 과잉으로 오탐지됨
            ...
            const sihwaQty = row.product.locationStock?.['시화'] || row.product.locationStock?.['서울'] || 0;
            const canInterTransferSihwa = row.shQty < safeStock && sihwaQty > 0;

            let procurementCategory: 'DOUBLE_STOCKOUT' | 'SURGING_DEMAND' | 'SIHWA_UNMET' | 'EXCESS' | 'STABLE' = 'STABLE';
            let procurementReason = "";

            if (row.shQty === 0 && row.ysQty === 0 && (row.recent90dSales > 0 || row.recent60dOrderCount > 0)) {
                procurementCategory = 'DOUBLE_STOCKOUT';
                procurementReason = "🚨 자사·공급처 동시 결품 (최근 주문/견적 수요 존재) - 긴급 수급 필요";
            } else if (row.shQty < safeStock && recommendedQty > 0) {
                procurementCategory = 'SIHWA_UNMET';
                if (canInterTransferSihwa) {
                    const transferQty = Math.min(finalDeficit, sihwaQty);
                    procurementReason = `🚛 시화창고 이송 권장 (${transferQty}개 보유) - 신규 매입 대신 시화창고에서 재고 이송 가능`;
                } else {
                    procurementReason = `⚠️ 부산재고(${row.shQty}개)가 적정 안전재고(${safeStock}개) 미달 - ${recommendedQty}개 선발주 권장`;
                }
            } else if (isExcessStock) {
                procurementCategory = 'EXCESS';
                procurementReason = `📉 직전 수요 대비 과잉재고 (${row.shQty}개 보유) - 추가 발주 보류 및 소진 검토`;
            } else {
            ...
            return {
                ...
                procurementCategory,
                procurementReason,
                recent90dOrderCount: row.recent60dOrderCount,
            };
        });
    ...
    const daekyungStockAverages = useMemo(() => {
        let items: DaekyungStockAnalysisItem[] = daekyungBaseStockAverages.map(r => {
            const busanRow = baseAnalyzedInventoryMap.get(r.id);
            const shQty = busanRow?.shQty ?? 0;
            const safeStock = busanRow?.safeStock ?? 0;
            const recommendedQty = busanRow?.recommendedQty ?? 0;
            const isDoubleStockoutWithDemand = busanRow?.isDoubleStockoutWithDemand ?? false;
            const isSurgingDemand = busanRow?.isSurgingDemand ?? false;
            const isExcessStock = busanRow?.isExcessStock ?? false;
            const isDeadStock = busanRow?.isDeadStock ?? false;
            const healthGrade = busanRow?.healthGrade ?? 'N';
            const turnoverRate = busanRow?.turnoverRate ?? 0;
        ...
        if (dkFilterProcurement !== 'ALL' && !isSearchOrSpecificFiltered) {
            items = items.filter(r => {
                ...
                if (dkFilterProcurement === 'SURGING_RECOMMENDED') {
                    return (r.isSurgingDemand === true || cat === 'SURGING_DEMAND') && recQty > 0;
                }
                if (dkFilterProcurement === 'SIHWA_UNMET') {
                    return cat === 'SIHWA_UNMET' || (r.shQty < r.safeStock && recQty > 0);
                }
                if (dkFilterProcurement === 'EXCESS') {
                    return cat === 'EXCESS' || r.isExcessStock === true;
                }
                if (dkFilterProcurement === 'STABLE') {
                    return cat === 'STABLE';
                }
                if (dkFilterProcurement === 'DROP_ITEM') {
                    return r.anomalyType === 'DROP';
                }
                if (dkFilterProcurement === 'SURGE_ITEM') {
                    return r.anomalyType === 'SURGE';
                }
                return true;
            });
        }
        ...
        if (dkViewMode === 'ITEM') {
            ...
            const materialGroups: Record<string, DaekyungStockAnalysisItem> = {};
        ...
        return finalResults.sort((a, b) => {
            ...
            switch (dkSortConfig.key) {
                ...
                case 'healthGrade': return ((a.healthGrade || 'N').localeCompare(b.healthGrade || 'N')) * dir;
                case 'turnoverRate': return ((a.turnoverRate || 0) - (b.turnoverRate || 0)) * dir;
                case 'quoteCount': return ((a.quoteCount || 0) - (b.quoteCount || 0)) * dir;
                case 'recent90dOrderCount': return ((a.recent90dOrderCount || 0) - (b.recent90dOrderCount || 0)) * dir;
                default: return 0;
            }
        });
    }, [daekyungBaseStockAverages, baseAnalyzedInventoryMap, dkSearchQuery, searchTerm, dkFilterItem, dkFilterMaterial, dkFilterSize, dkFilterHealthGrade, dkFilterProcurement, dkViewMode, dkSortConfig]);

    const daekyungStats = useMemo(() => {
        const totalCurrentStock = daekyungStockAverages.reduce((sum, item) => sum + item.currentStock, 0);
        const totalAvg1m = daekyungStockAverages.reduce((sum, item) => sum + (item.avg1m || 0), 0);
    ...
    const handleCreateOrder = (selectedSet: Set<string>, listType: 'CRITICAL' | 'WARNING' | 'REGULAR') => {
        ...
        const titleMap = {
            CRITICAL: '선발주 필요 품목 발주 수량 설정 (부산)',
            WARNING: '일반 발주 필요 품목 발주 수량 설정 (부산)',
            REGULAR: '정기 발주 필요 품목 발주 수량 설정 (부산)'
        };
    ...
    const handleCreateGlobalOrder = () => {
        ...
        setBatchOrderModalTitle(`통합 선택 품목 일괄 발주 수량 설정 (총 ${modalList.length}건 - 부산)`);
        setBatchOrderModalItems(modalList);
        setIsBatchOrderModalOpen(true);
    };
    ...
    const handleCreateManualOrder = () => {
        ...
        setBatchOrderModalTitle(`수동 선택 품목 발주 수량 설정 (총 ${modalList.length}건 - 부산)`);
        setBatchOrderModalItems(modalList);
        setIsBatchOrderModalOpen(true);
    };
    ...
    const healthDiagnosis = useMemo(() => {
        ...
        // ── 결품 기회손실: 대경+부산 모두 0일 때 주문→취소 이력 ────
        ...
        orders.forEach(o => {
            ...
            if (custStr.includes('재고') || custStr.includes('부산') || custStr.includes('서울') || custStr.includes('알트에프') || custStr.includes('altf')) return;
    ...
    const handleExportSihwaSummary = () => {
        const headers = ['품목', '두께', '사이즈', '재질', '현재재고(부산)', '입고예정(미결결과)', '발주서번호(들)', '발주날짜(들)', '입고예정일'];
        ...
        analyzedInventory.forEach(row => {
            ...
            orders.forEach(order => {
                ...
                const isSihwaIncoming = targetCustomer.includes('재고') || targetCustomer.includes('서울') || targetCustomer.includes('부산') || targetCustomer.includes('알트에프') || targetCustomer.includes('altf');
        ...
        link.setAttribute('download', `부산재고_입고대기분석_${dateStr}.csv`);
        ...
        <div className="space-y-6 animate-in fade-in duration-500 pb-20">
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-slate-300 text-[13px] mt-1 pr-4">
                            현재 부산재고 출고량의 대부분을 '수도권(경기) 판매'로 간주하여 분석 중입니다.<br />
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4 xl:gap-5">
                ...
                >
                    ...
                    <p className="text-sm font-medium opacity-80 z-10 mt-auto">현재 보유 중인 부산재고 전체의 실매입 추정 자산가치입니다 (Stubend 제외)</p>
                </div>
                                                    ...
                                                    <table className="w-full text-sm text-left whitespace-nowrap">
                                                        <thead className="bg-slate-50 text-slate-500 font-bold border-y border-slate-100 select-none sticky top-0 z-10 shadow-sm">
                                                            <tr>
                                                                ...
                                                                <th className="px-5 py-3 text-right cursor-pointer hover:bg-slate-200 transition" onClick={() => handleSort('shQty')}>부산재고 {sortConfig.key === 'shQty' && (sortConfig.direction === 'asc' ? '↑' : '↓')}</th>
...
<th className="px-4 py-3 w-10 text-center">
                                                                ...
                                                                <th className="px-3 py-3 w-10 text-center">핀</th>
                                                                <th className="px-5 py-3 cursor-pointer hover:bg-slate-200 transition" onClick={() => handleSort('id')}>품목 코드 {sortConfig.key === 'id' && (sortConfig.direction === 'asc' ? '↑' : '↓')}</th>
                                                                <th className="px-5 py-3 text-right">부산재고</th>
...
<tfoot className="bg-amber-50/50 border-t-2 border-amber-200">
                                                ...
                                                ) : <div className="p-8 text-center text-slate-400">발주가 필요한 품목이 없습니다. 부산재고 관리가 매우 이상적입니다!</div>}
                                            </div>
                                        )}
                                    </div>
                                        ...
                                        <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden flex flex-col h-fit">
                                            <div className="bg-slate-800 text-white px-5 py-4 flex items-center justify-between shrink-0">
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <h2 className="font-bold">월별 부산재고 보충(매입) 누적</h2>
                                                </div>
                                                <select
                                                    title="월 선택"
                                                    aria-label="월 선택"
                                                                                        ...
                                                                                        {isExpanded && analysis && (
                                                                                            <div className="p-3 bg-slate-50/50 border-t border-slate-100 grid grid-cols-2 gap-4">
                                                                                                <div className="flex flex-col gap-1">
                                                                                                    <span className="text-[10px] text-slate-400 font-bold">현재재고(부산)</span>
...
<th className="px-3 py-3 w-10 text-center border-r border-slate-200">
                                                ...
                                                <th className="px-4 py-3 text-right cursor-pointer hover:bg-slate-200 transition text-indigo-700" onClick={() => handleSort('shQty')}>부산재고 {sortConfig.key === 'shQty' && (sortConfig.direction === 'asc' ? '↑' : '↓')}</th>
                                                ...
                                                <th className="px-4 py-3 text-right cursor-pointer hover:bg-slate-200 transition group relative" onClick={() => handleSort('daysOnHand')}>
                                                    ...
                                                    <div className="absolute right-0 bottom-full mb-2 w-56 bg-slate-800 text-white text-[11px] p-3 rounded shadow-lg hidden group-hover:block z-50 text-left font-normal whitespace-normal cursor-auto">
                                                        ...
                                                        <div className="text-slate-300 mb-1">현재 부산재고를 <span className="font-bold">일평균 판매량</span>으로 나눈 값으로, <span className="text-amber-300 font-bold">재발주 시점 산출 및 결품 예방</span>을 위해 사용됩니다.</div>
...
════════════════════════════════════════════════ */}
                                            ...
                                            <div className="px-5 py-4 bg-linear-to-r from-violet-50 to-indigo-50 border-b border-violet-200 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                                                <div>
                                                    ...
                                                    <div className="text-[10px] text-violet-500 mt-0.5">
                                                        부산 및 대경재고가 없는 상태에서 고객의 견적 문의나 주문 시도 후 취소·철회된 이력 분석
                                                    </div>
                                                </div>
                                                    ...
                                                    <table className="w-full text-left border-collapse">
                                                        <thead>
                                                            <tr className="bg-slate-100 border-b border-slate-200 text-slate-600 text-[10px] font-bold">
                                                                ...
                                                                <th className="p-3 text-right">부산재고</th>
                                                                ...
                                                                >
                                                                    ...
                                                                    <div className="text-[10px] text-slate-600 grid grid-cols-2 gap-x-2 gap-y-1 mb-3 bg-slate-50 p-2 rounded-lg font-mono">
                                                                        <div className="flex justify-between">
                                                                            <span className="text-slate-400 font-sans">부산재고:</span>
                            ...
                            {activeTab === 'DAEKYUNG_STOCK' && (
                                <div className="space-y-6 p-4 md:p-0 pb-8 animate-in fade-in duration-300">
                                    ...
                                    <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
                                        ...
                                        <div className="bg-blue-50/50 border-2 border-blue-300 rounded-2xl p-3.5 shadow-xs relative">
                                            <div className="absolute -top-2 right-3 bg-blue-600 text-white text-[9px] font-bold px-2 py-0.5 rounded-full shadow">📌 기준 지표</div>
                                            ...
                                            <div className="text-[9px] text-blue-400 mt-1 font-medium">
                                                최근 30일 중 확정일 {daekyungCoverage.confirmedDaysLast30}일
                                                {daekyungCoverage.daysSinceLastConfirm !== null && ` · 마지막 확정 ${daekyungCoverage.daysSinceLastConfirm}일 전`}
                                            </div>
                                        </div>
                                    ...
                                    <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 text-white flex flex-col md:flex-row md:items-center justify-between gap-4">
                                        <div className="space-y-1">
                                            ...
                                            <p className="text-[11px] text-slate-300 leading-relaxed">
                                                데이터 갱신은 불특정일에 진행됩니다. 최근 30일 중 실제로 재고가 확정된 날은 <strong>{daekyungCoverage.confirmedDaysLast30}일</strong>
                                                {daekyungCoverage.lastConfirmedDate && daekyungCoverage.daysSinceLastConfirm !== null && (
                                                    <>이며, 마지막 확정은 <strong>{daekyungCoverage.daysSinceLastConfirm}일 전({daekyungCoverage.lastConfirmedDate})</strong>입니다.</>
                                                )} 그래서 일별 단기 변동량 파악을 위한 <strong>1개월(30일) 평균 재고</strong>를 기준 지표로 삼고, 중장기 흐름을 위한 <strong>3개월/6개월 평균 재고</strong>는 보조로 함께 참고하는 것이 합당합니다.
                                                각 품목이 전체 대경 재고에서 차지하는 <strong>상대적 비중(Relative Share)</strong>을 비교 분석하여 앞으로 상시 보유해야 할 품목을 관리를 진행하세요.
                                            </p>
                                        </div>
                                    </div>

                                    {/* ── 2-1. 급감/급증 이상치 분석 (최근 30일 평균 vs 직전 30일 평균) ── */}
                                    {(daekyungDropItems.length > 0 || daekyungSurgeItems.length > 0) && (
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                            <div className="bg-rose-50/60 border border-rose-200 rounded-2xl p-4">
                                                <h4 className="text-xs font-black text-rose-700 flex items-center gap-1.5 mb-2">🔻 급감 품목 Top 5</h4>
                                                {daekyungDropItems.length === 0 ? (
                                                    <div className="text-[11px] text-rose-400 py-2">최근 급감 품목이 없습니다.</div>
                                                ) : (
                                                    <div className="space-y-1.5">
                                                        {daekyungDropItems.map(item => (
                                                            <button
                                                                key={item.id}
                                                                onClick={() => setDkFilterProcurement('DROP_ITEM')}
                                                                className="w-full flex items-center justify-between bg-white/70 hover:bg-white border border-rose-100 rounded-lg px-3 py-1.5 text-left transition-colors"
                                                            >
                                                                <div className="min-w-0">
                                                                    <div className="text-[11px] font-bold text-slate-700 truncate">{item.name}</div>
                                                                    <div className="text-[9px] text-slate-400">{item.prev1mAvg}개 → {item.avg1m}개 · 현재고 {item.currentStock}개</div>
                                                                </div>
                                                                <div className={`text-xs font-black shrink-0 ml-2 ${item.anomalySeverity === 'HIGH' ? 'text-rose-700' : 'text-rose-500'}`}>
                                                                    {item.changePct1m}% ({item.changeQty1m > 0 ? '+' : ''}{item.changeQty1m})
                                                                </div>
                                                            </button>
                                                        ))}
                                                    </div>
                                            ...
                                            <div className="bg-emerald-50/60 border border-emerald-200 rounded-2xl p-4">
                                                <h4 className="text-xs font-black text-emerald-700 flex items-center gap-1.5 mb-2">🔺 급증 품목 Top 5</h4>
                                                {daekyungSurgeItems.length === 0 ? (
                                                    <div className="text-[11px] text-emerald-400 py-2">최근 급증 품목이 없습니다.</div>
                                                ) : (
                                                    <div className="space-y-1.5">
                                                        {daekyungSurgeItems.map(item => (
                                                            <button
                                                                key={item.id}
                                                                onClick={() => setDkFilterProcurement('SURGE_ITEM')}
                                                                className="w-full flex items-center justify-between bg-white/70 hover:bg-white border border-emerald-100 rounded-lg px-3 py-1.5 text-left transition-colors"
                                                            >
                                                                <div className="min-w-0">
                                                                    <div className="text-[11px] font-bold text-slate-700 truncate">{item.name}</div>
                                                                    <div className="text-[9px] text-slate-400">{item.prev1mAvg}개 → {item.avg1m}개 · 현재고 {item.currentStock}개</div>
                                                                </div>
                                                                <div className={`text-xs font-black shrink-0 ml-2 ${item.anomalySeverity === 'HIGH' ? 'text-emerald-700' : 'text-emerald-500'}`}>
                                                                    +{item.changePct1m}% ({item.changeQty1m > 0 ? '+' : ''}{item.changeQty1m})
                                                                </div>
                                                            </button>
                                                        ))}
                                        ...
                                        <div className="grid grid-cols-1 sm:grid-cols-4 gap-3">
                                            <div className="flex flex-col gap-1 sm:col-span-2">
                                                ...
                                                >
                                                    ...
                                                    <option value="SIHWA_UNMET">⚠️ 부산재고 미달 (선발주 권장)</option>
                                            ...
                                            <table className="w-full text-xs text-left whitespace-nowrap">
                                                <thead className="bg-slate-50 text-slate-500 font-bold border-b border-slate-100 sticky top-0 z-10 shadow-sm">
                                                    <tr>
                                                        ...
                                                        <th className="px-4 py-3 text-right cursor-pointer hover:bg-slate-100 transition" onClick={() => setDkSortConfig(prev => ({ key: 'safeStock', direction: prev.key === 'safeStock' && prev.direction === 'desc' ? 'asc' : 'desc' }))}>
                                                            부산 안전재고량 {dkSortConfig.key === 'safeStock' && (dkSortConfig.direction === 'asc' ? '↑' : '↓')}
                                                        </th>
                                                ...
                                                <tbody className="divide-y divide-slate-100">
                                                    {daekyungStockAverages.map((row, index) => {
                                                        ...
                                                        return (
                                                            <tr key={row.id} className={`hover:bg-slate-50/80 transition cursor-pointer ${isSelected ? 'bg-indigo-50/50' : ''}`}>
                                                                {dkViewMode === 'ITEM' && (
                                                                    ...
                                                                    <>
                                                                        ...
                                                                        <td className="px-4 py-2.5 text-center">
                                                                            <span className={`px-2 py-0.5 rounded text-[10px] sm:text-xs font-black ${
                                                                                (busanRow?.healthGrade || row.healthGrade) === 'A' ? 'bg-emerald-100 text-emerald-700' :
                                                                                (busanRow?.healthGrade || row.healthGrade) === 'B' ? 'bg-blue-100 text-blue-700' :
                                                                                (busanRow?.healthGrade || row.healthGrade) === 'C' ? 'bg-amber-100 text-amber-700' :
                                                                                (busanRow?.healthGrade || row.healthGrade) === 'D' ? 'bg-orange-100 text-orange-700' :
                                                                                (busanRow?.healthGrade || row.healthGrade) === 'E' ? 'bg-rose-100 text-rose-700' :
                                                                                'bg-slate-100 text-slate-500'
                                                                            }`} title="종합 건전성 등급">
                                                                                {(busanRow?.healthGrade || row.healthGrade || 'N')}급
                                                                            </span>
                                                                        </td>
                                                                        <td className="px-4 py-2.5 text-center">
                                                                            {(busanRow?.healthGrade || row.healthGrade) && (busanRow?.healthGrade || row.healthGrade) !== 'N' ? (
                                                                                <div className="flex flex-col items-center">
                                                                                    <span className={`text-[10px] font-black px-1.5 rounded ${
                                                                                        (busanRow?.healthGrade || row.healthGrade) === 'A' ? 'bg-emerald-100 text-emerald-700' :
                                                                                        (busanRow?.healthGrade || row.healthGrade) === 'B' ? 'bg-amber-100 text-amber-700' :
                                                                                        (busanRow?.healthGrade || row.healthGrade) === 'C' ? 'bg-blue-100 text-blue-600' :
                                                                                        'bg-rose-100 text-rose-500'
                                                                                    }`}>
                                                                                        {(busanRow?.healthGrade || row.healthGrade)}급
                                                                                    </span>
                                                                                    <span className="text-[10px] font-mono text-slate-400 mt-0.5">
                                                                                        {(busanRow?.turnoverRate ?? row.turnoverRate ?? 0) > 0 ? `${(busanRow?.turnoverRate ?? row.turnoverRate)}x` : ''}
                                                                                    </span>
                                                                                </div>
                                                                            ) : <span className="text-slate-200">—</span>}
                                                                        </td>
                                                                        <td className="px-4 py-2.5 text-center text-slate-600">
                                                                            <div className="flex flex-col items-center gap-1">
                                                                                <span className="font-black text-slate-800 text-[12px]">
                                                                                    {(busanRow?.recent90dSales ?? row.recent90dSales ?? 0).toLocaleString()}
                                                                                    <span className="text-[10px] text-slate-500 font-bold ml-0.5 mr-1">개 /</span>
                                                                                    {(busanRow?.recent90dOrderCount ?? row.recent90dOrderCount ?? 0).toLocaleString()}
                                                                ...
                                                                <td className="px-4 py-2.5 text-right font-bold text-slate-700 font-mono">
                                                                    {row.safeStock.toLocaleString()}개
                                                                </td>
                                                                <td className="px-4 py-2.5 text-right font-black font-mono text-indigo-700">
                                                                    {row.recommendedQty > 0 ? `${row.recommendedQty.toLocaleString()}개` : '—'}
                                                                </td>
                                                                <td className="px-4 py-2.5 text-left text-xs font-bold text-slate-700">
                                                                    <span className={`px-2 py-0.5 rounded text-[11px] font-bold ${
                                                                        row.procurementReason.includes('🚨') ? 'bg-rose-100 text-rose-700 border border-rose-200' :
                                                                        row.procurementReason.includes('🚛') ? 'bg-indigo-100 text-indigo-800 border border-indigo-200 shadow-xs' :
                                                                        row.procurementReason.includes('⚠️') ? 'bg-amber-100 text-amber-800 border border-amber-200' :
                                                                        row.procurementReason.includes('📉') ? 'bg-slate-100 text-slate-600 border border-slate-200' :
                                                    ...
                                                    {daekyungStockAverages.length === 0 && (
                                                        <tr>
                                                            <td colSpan={dkViewMode === 'ITEM' ? 12 : 8} className="px-4 py-16 text-center text-slate-400 font-medium">
                                                                조건에 해당하는 대경재고 품목이 없습니다.
                                                            </td>
                                                        </tr>
                                                    )}
                                                </tbody>
            ...
            {/* 부산재고 선택 품목 일괄 발주 수량 확인/수정 팝업 모달 */}
                    ...
                    <div className="bg-white rounded-2xl shadow-2xl border border-slate-200 w-full max-w-4xl max-h-[85vh] flex flex-col overflow-hidden animate-in fade-in zoom-in duration-200 text-slate-800">
                        ...
                        <div className="p-6 overflow-y-auto flex-1 space-y-6">
                            <div className="bg-blue-50 border border-blue-200 p-4 rounded-xl text-blue-800 text-xs font-bold leading-relaxed">
                                💡 자동 계산된 부산 창고 대경 제품의 일일 변동 데이터입니다. 
                            ...
                            <div className="pb-4">
                                ...
                                {pendingSihwaList.length === 0 ? (
                                    <p className="text-xs text-slate-400 py-4 text-center font-medium">부산재고의 변동 감지 항목이 없습니다.</p>
                                ) : (
