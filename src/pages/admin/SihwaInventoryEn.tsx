...
// ── Inventory Health Diagnosis 기준 (쿠팡·다이소 물류 기준 참고) ──────────
const DEAD_STOCK_DAYS        = 90;    // 무판매 N일 이상 = Dead Stock
const DEAD_STOCK_TURNOVER    = 0.5;   // 회전율 이 이하 = Dead Stock
const EXCESS_STOCK_RATIO     = 2.0;   // 목표재고의 N배 초과 = 과잉
const EXCESS_DAYS_THRESHOLD  = 180;   // 잔여일 N일 이상 = 과잉
const SLOW_MOVE_DAYS_MIN     = 90;    // Slow-moving 잔여일 하한
const SLOW_MOVE_DAYS_MAX     = 180;   // Slow-moving 잔여일 상한
const HEALTHY_DEAD_RATIO     = 0.05;  // 허용 Dead Stock 비중 (5%)
const HEALTHY_EXCESS_RATIO   = 0.10;  // 허용 Excess Stock 비중 (10%)
...
export default function SihwaInventory() {
    ...
    const healthDiagnosis = useMemo(() => {
      ...
      // ── 즉시 처분 대상 (Dead Stock 중 대경 반품 가능 여부 기준) ──
...
{activeTab === 'HEALTH_DIAGNOSIS' && (
  <div className="space-y-5 p-4 md:p-0 pb-8 animate-in fade-in duration-300">
    ...
    <div className="grid grid-cols-1 xl:grid-cols-[280px_1fr] gap-4">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex-1">
            ...
            <div className="text-[11px] text-slate-500 space-y-0.5">
              <div>Dead Stock <span className="font-bold text-rose-600">{(healthDiagnosis.deadRatio * 100).toFixed(1)}%</span></div>
              <div>Excess Stock <span className="font-bold text-orange-500">{(healthDiagnosis.excessRatio * 100).toFixed(1)}%</span></div>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
        <div className="text-xs font-black text-slate-500 uppercase tracking-widest mb-3">
          Total Inventory Asset Composition ({formatCur(Math.round(healthDiagnosis.totalStockValue / 10000))}만원)
        </div>
        {/* 스택 바 */}
        <div className="h-7 rounded-lg overflow-hidden flex mb-3">
          {[
            { value: healthDiagnosis.optimalStockValue,  bgClass: 'bg-green-600', label: 'Optimal/Safety' },
        ...
        <div className="flex flex-wrap gap-3 mb-4">
          {[
            { bgClass:'bg-green-600', label:'Optimal/Safety', value: healthDiagnosis.optimalStockValue },
            { bgClass:'bg-orange-600', label:'Excess Stock ⚠', value: healthDiagnosis.excessStockValue },
            { bgClass:'bg-rose-600', label:'Dead Stock 🚨', value: healthDiagnosis.deadStockValue },
            { bgClass:'bg-purple-600', label:'Slow-moving', value: healthDiagnosis.slowMoveValue },
          ].map(l => (
    ...
    <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
      {[
        { emoji:'☠️', label:'Dead Stock 품목', value:`${healthDiagnosis.deadStockItems.length}개`, sub:'90일+ 무판매 or 0.5x미만', color:'rose', border:'border-rose-400' },
        { emoji:'📦', label:'Excess Stock 품목', value:`${healthDiagnosis.excessStockItems.length}개`, sub:'목표재고 200%+ 초과', color:'orange', border:'border-orange-400' },
        { emoji:'🐌', label:'Slow-moving 품목', value:`${healthDiagnosis.slowMoveItems.length}개`, sub:'D등급·잔여 90~180일', color:'purple', border:'border-purple-400' },
    ...
    <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
      ...
      <div className="bg-white rounded-2xl border border-rose-200 shadow-sm overflow-hidden">
        <div className="px-5 py-3 bg-rose-50 border-b border-rose-200 flex items-center justify-between">
          <div>
            <div className="text-sm font-black text-rose-800">☠️ Dead Stock 상세 — 즉시 조치 필요</div>
        ...
        <div className="overflow-auto max-h-80">
          <table className="w-full text-xs text-left whitespace-nowrap">
            ...
            <tbody className="divide-y divide-slate-100">
              ...
              {healthDiagnosis.deadStockItems.length === 0 && (
                <tr><td colSpan={7} className="px-4 py-8 text-center text-slate-400">Dead Stock가 없습니다 🎉</td></tr>
              )}
            </tbody>
          </table>
        </div>
      ...
      <div className="bg-white rounded-2xl border border-orange-200 shadow-sm overflow-hidden">
        <div className="px-5 py-3 bg-orange-50 border-b border-orange-200 flex items-center justify-between">
          <div>
            <div className="text-sm font-black text-orange-800">📦 Excess Stock 상세 — 발주 일시 중단 권고</div>
          ...
          <table className="w-full text-xs text-left whitespace-nowrap">
            ...
            <tbody className="divide-y divide-slate-100">
              ...
              {healthDiagnosis.excessStockItems.length === 0 && (
                <tr><td colSpan={7} className="px-4 py-8 text-center text-slate-400">Excess Stock가 없습니다 🎉</td></tr>
              )}
            </tbody>
          </table>
        </div>
    ...
    <div className="bg-linear-to-br from-slate-800 to-indigo-900 rounded-2xl p-5 text-white">
      ...
      <div className="text-[11px] text-slate-300 mb-4">아래 데이터가 누적되면 악성·Excess Stock 판단 정확도가 90%↑로 향상됩니다</div>
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-5 gap-3">
        {[
          ...
          { phase:'3개월 후', textClass:'text-amber-600', bgClass:'bg-amber-600', items:['권역별 수요 패턴 분리','하치장 품목 기준 데이터화','Dead Stock 사전 예측 모델'] },
