...
export const AdminOrderDetail = memo(function AdminOrderDetail({ order, onClose, onUpdate, initialMode = 'CUSTOMER' }: AdminOrderDetailProps) {
    ...
    }, [orders, poEndCustomer, order.customerName, editableCustomerInfo.bizNo, order.userId, crmCustomers]);
    ...
    const [items, setItems] = useState<LineItem[]>(() => {
        ...
        return initialItems.map(item => {
            ...
            // 6월 15일 이후 주문 건이고 할인율 정보가 누락되었거나 0인 경우 요율 자동 유추 및 인벤토리 매칭
            if (isOrderAfterEffectiveDate && (discountRate === undefined || discountRate === 0)) {
